...
class UpdateCommand extends Command<int> {
  ...
  FutureOr<int> run() async {
    ...
    final res = await http.get(
      Uri.parse(
        'https://github.com/open-weather-vision/owvision/releases/latest/download/owvi_$arch',
      ),
    );
    ...
    if (Platform.isLinux) {
      ...
      final wasRunning = await daemon.isActive();
      if (wasRunning) {
        await daemon.stop();
      ...
      PrettyPrinter.output(
        Box([
          Text(
            chalk.green(
              "Upgraded ${chalk.bold("owvi")} to the latest version!",
            ),
          ),
        ]),
      );
